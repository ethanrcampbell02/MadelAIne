...
lines = {
    ...
    "avg_max_q": ax3.plot([], [], label="Avg Max Q-Value", color='purple')[0]
}
...
ax3.set_ylabel("Max Q-Value")
ax3.legend()
ax3.set_title("Average Max Q-Value on Holdout States")

# Adjust spacing between subplots to prevent overlap
fig.tight_layout(pad=3.0)  # Add more padding between subplots
# Alternative: Use subplots_adjust for more fine-grained control
...
def save_plot_and_values(epoch=None):
    ...
    np.save(os.path.join(stats_subdir, f'avg_max_q_values{"_epoch_" + str(epoch) if epoch is not None else ""}.npy'), np.array(avg_max_q_values))
...
for epoch in trange(NUM_OF_EPOCHS, desc="Epochs"):
    ...
    logging.info("Calculating average max Q-value for holdout states...")
    avg_max_q = np.mean([agent.predict_max_q(state) for state in holdout_states])
    ...
    lines["avg_max_q"].set_data(epochs, avg_max_q_values)
    ax1.relim()
    ax1.autoscale_view()
    ax2.relim()
    ax2.autoscale_view()
    ...
    if SAVE_MODEL_AFTER_EPOCH:
        agent.save_model(os.path.join(model_subdir, "model_" + str(epoch + 1) + "_epoch.pt"))
